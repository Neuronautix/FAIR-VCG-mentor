...
EQIPD also participates in the **cross-standard crosswalk** and **paper-suggestion** flows so the whole workflow is connected:
- *Cross-standard auto-satisfaction.* Five EQIPD requirements declare `crosswalk` lists pointing at ARRIVE/PREPARE field_ids the user may already have filled: CR4→`ethics_statement`/`prepare_legislation_compliance`, CR10→`prepare_clear_hypothesis`, CR11→`personnel_training`/`prepare_staff_competence`, CR12→`procedures_description`, CR15→`prepare_risk_assessment`. To make this work for a standalone standard, `validate_against_template` Step B now treats a crosswalk target as satisfied when it is a satisfied sibling field **or** when it has a non-empty value in the (shared) `session["metadata"]` — so EQIPD auto-satisfies from fields belonging to other standards. This is additive: existing ARRIVE/PREPARE crosswalks reference in-template fields and behave identically. The other 13 requirements (data integrity, storage, traceability, sustainability, …) have no honest equivalent and stay manual/LLM-filled.
- *Paper-scoring.* `_score_template_from_paper` has an `eqipd` branch: a modest boost on quality/rigour/reproducibility/SOP/data-integrity keywords plus a small preclinical-in-vivo bonus, so EQIPD surfaces as a **low-ranked** candidate on the Paper Import page for any in vivo paper. It never outranks a true reporting template and (having no `required_columns`) never auto-assigns on upload.
