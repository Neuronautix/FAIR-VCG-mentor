...
def generate_fair_report_md(session: Dict) -> bytes:
    """Generate fair_readiness_report.md from a session dict."""
    import_info = session.get("import_info", {})
    columns = session.get("columns", [])
    table_structure = session.get("table_structure", {})
    fair_score = session.get("fair_score", {})
    metadata = session.get("metadata", {})
    issues = session.get("issues", [])
    content = generate_fair_report(import_info, columns, table_structure, fair_score, metadata, issues)
    return content.encode("utf-8") if isinstance(content, str) else content


def generate_arrive_report_md(session: Dict) -> bytes:
    """Generate arrive_conformance_report.md from a session dict.

    This is a reporting completeness assessment, not a quality validation.
    """
    template_validation = session.get("template_validation", [])
    metadata = session.get("metadata", {})
    import_info = session.get("import_info", {})
    ...
    lines += [
        "# ARRIVE 2.0 Reporting Completeness Assessment",
        "",
        f"**Dataset:** {title}",
        f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        f"**Template:** {template_id or 'No ARRIVE template assigned'}",
        "",
        "> **Note:** This is a reporting completeness assessment only — it indicates "
        "whether the required ARRIVE 2.0 fields are present in your metadata. "
        "It does not validate the quality, accuracy, or completeness of your research, "
        "and does not constitute regulatory certification.",
        "",
    ]

    arrive_entries = [
        e for e in template_validation
        if e.get("arrive_section") or (e.get("standard") or "").lower().startswith("arrive")
    ]

    if not arrive_entries:
        lines += [
            "## No ARRIVE Template Assigned",
            "",
            "No ARRIVE 2.0 template has been assigned to this dataset. "
            "Go to the Templates page to assign the ARRIVE 2.0 template and run a conformance check.",
            "",
        ]
    else:
        satisfied = [e for e in arrive_entries if e.get("status") == "satisfied"]
        missing = [e for e in arrive_entries if e.get("status") == "missing"]
        partial = [e for e in arrive_entries if e.get("status") == "partial"]

        pct = round(100 * len(satisfied) / len(arrive_entries)) if arrive_entries else 0
        lines += [
            "## Summary",
            "",
            f"| Metric | Count |",
            f"|---|---|",
            f"| Total ARRIVE fields | {len(arrive_entries)} |",
            f"| Satisfied | {len(satisfied)} ({pct}%) |",
        ...
        if satisfied:
            lines += ["## Satisfied Fields", ""]
            for e in satisfied:
                section = e.get("arrive_section") or e.get("section") or "—"
                sb = e.get("satisfied_by") or {}
                via = " (via crosswalk)" if sb.get("via_crosswalk") else ""
                lines.append(f"- **{e['field_id']}** ({section}){via}")
            lines.append("")
        ...
        if missing:
            ...
            for e in missing:
                ...
                lines.append(f"- **{e['field_id']}** ({section}) — severity: {sev}")
            lines.append("")

        if partial:
            lines += ["## Partial Fields", ""]
            for e in partial:
                section = e.get("arrive_section") or e.get("section") or "—"
                lines.append(f"- **{e['field_id']}** ({section})")
            lines.append("")

    lines += [
        "---",
        "*Generated by FAIR CSV Mentor — reporting completeness assessment only, not regulatory certification.*",
    ]
    return "\n".join(lines).encode("utf-8")


def generate_prepare_report_md(session: Dict) -> bytes:
    """Generate prepare_readiness_report.md from a session dict.

    This is a study planning readiness assessment, not regulatory validation.
    """
    template_validation = session.get("template_validation", [])
    metadata = session.get("metadata", {})
    import_info = session.get("import_info", {})
    ...
    title = metadata.get("title") or import_info.get("filename", "Untitled")
    lines += [
        "# PREPARE Study Planning Readiness Assessment",
        "",
        f"**Dataset:** {title}",
        f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        f"**Template:** {template_id or 'No PREPARE template assigned'}",
        "",
        "> **Note:** This is a study planning readiness assessment (Smith AJ et al., Lab Anim 2018). "
        "It indicates whether the PREPARE planning checklist items are addressed in your metadata. "
        "It does not validate experimental quality or constitute regulatory certification.",
        "",
    ]

    prepare_entries = [
        e for e in template_validation
        if e.get("prepare_section") or (e.get("standard") or "").lower().startswith("prepare")
    ]

    if not prepare_entries:
        lines += [
            "## No PREPARE Template Assigned",
            "",
            "No PREPARE template has been assigned to this dataset. "
            "Go to the Templates page to assign the PREPARE or ARRIVE-PREPARE crosswalk template.",
            "",
        ]
    else:
        satisfied = [e for e in prepare_entries if e.get("status") == "satisfied"]
        missing = [e for e in prepare_entries if e.get("status") == "missing"]
        partial = [e for e in prepare_entries if e.get("status") == "partial"]

        pct = round(100 * len(satisfied) / len(prepare_entries)) if prepare_entries else 0
        lines += [
            "## Summary",
            "",
            "| Metric | Count |",
            "|---|---|",
            f"| Total PREPARE items | {len(prepare_entries)} |",
            f"| Addressed | {len(satisfied)} ({pct}%) |",
            f"| Not addressed | {len(missing)} |",
            f"| Partial | {len(partial)} |",
            "",
        ]

        if satisfied:
            lines += ["## Addressed Items", ""]
            for e in satisfied:
                section = e.get("prepare_section") or e.get("section") or "—"
                sb = e.get("satisfied_by") or {}
                via = " (via crosswalk)" if sb.get("via_crosswalk") else ""
                lines.append(f"- **{e['field_id']}** ({section}){via}")
            lines.append("")
        ...
        if missing:
            ...
            for e in missing:
                section = e.get("prepare_section") or e.get("section") or "—"
                sev = e.get("severity", "low")
                lines.append(f"- **{e['field_id']}** ({section}) — priority: {sev}")
            lines.append("")

    lines += [
        "---",
        "*Generated by FAIR CSV Mentor — study planning readiness assessment only, not regulatory certification.*",
    ]
    return "\n".join(lines).encode("utf-8")
...
def generate_fair_report(
    ...
    lines += [
        ...
        '- `arrive_conformance_report.md` — ARRIVE 2.0 reporting completeness assessment',
        '- `prepare_readiness_report.md` — PREPARE study planning readiness assessment',
        '',
        '---',
