...
def _apply_project_schema_defaults(
    ...
    """Fill wizard roles from the approved/validated study-corpus consensus schema.

    Runs before template defaults, so template-locked reporting-standard values
    still override schema suggestions. This is the bridge from the Study Corpus
    feature (many papers -> one consensus schema) into VCG configuration.
    """
